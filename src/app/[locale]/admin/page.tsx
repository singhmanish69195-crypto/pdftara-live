...
export default function PDFTaraAdmin() {
  ...
  const [preview, setPreview] = useState<string | null>(null); // Image preview ke liye
    ...
    <div className="min-h-screen bg-[#f8fafc] text-slate-800 font-sans">
      ...
      <header className="bg-white border-b sticky top-0 z-50 px-8 py-3 flex justify-between items-center shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <h1 className="font-bold text-blue-900 uppercase tracking-tighter">PDFTara Admin</h1>
        </div>
        ...
        {tab === 'editor' ? (
          <form onSubmit={handlePublish} className="flex flex-col lg:flex-row gap-8 text-left">
            ...
            <aside className="w-full lg:w-96 space-y-6">
              <div className="bg-white p-8 rounded-[2.5rem] border shadow-sm sticky top-[80px]">
                    ...
                    <label className="relative group cursor-pointer border-4 border-dashed border-slate-100 rounded-[2rem] h-56 flex flex-col items-center justify-center overflow-hidden hover:border-blue-200 transition-all bg-slate-50/50">
                      {preview ? (
                        ...
                        <div className="text-center">
                          <svg className="w-12 h-12 text-slate-300 mx-auto mb-2 group-hover:text-blue-400 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"></path></svg>
                          <p className="text-[10px] font-black text-slate-400">UPLOAD IMAGE</p>
                        </div>
                      )}
                ...
                <button type="submit" disabled={loading} className="w-full mt-10 bg-[#f8901c] hover:bg-[#e67e17] text-white py-4 rounded-full font-black shadow-lg shadow-orange-100 transition active:scale-95">
          ...
          <div className="bg-white p-10 rounded-[2.5rem] shadow-sm border space-y-4 min-h-[600px]">
            ...
            {posts.slice().reverse().map((post: any) => (
              <div key={post.slug} className="flex justify-between items-center p-6 bg-slate-50/50 rounded-3xl border border-slate-100 hover:bg-white transition-all group">
                <div className="flex items-center gap-4">
                  <img src={post.image} className="w-20 h-14 object-cover rounded-xl shadow-sm" alt="thumb" />
                  <div>
                    <h4 className="font-black text-lg text-slate-800">{post.title}</h4>
